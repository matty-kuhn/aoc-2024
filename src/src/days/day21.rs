...
fn walk_backwards_nums(input: &str) -> String {
    static KEYPAD: [[char; 3]; 4] = [
        ['7', '8', '9'],
        ['4', '5', '6'],
        ['1', '2', '3'],
    ...
    let mut start_coords = COORDS.get(&'A').unwrap().clone();
    ...
    for ch in input.chars() {
        match ch {
            '^' => {
                start_coords.1 -= 1;
                assert!(start_coords != *COORDS.get(&' ').unwrap());
            }
            'v' => {
                start_coords.1 += 1;
                assert!(start_coords != *COORDS.get(&' ').unwrap());
            ...
            'A' => {
                assert!(start_coords != *COORDS.get(&' ').unwrap());
                res.push(KEYPAD[start_coords.1][start_coords.0]);
            }
            _ => unreachable!(),
        }
    }
...
fn walk_backwards_dirs(input: &str) -> String {
    static KEYPAD: [[char; 3]; 2] = [[' ', '^', 'A'], ['<', 'v', '>']];
    static COORDS: LazyLock<HashMap<char, (usize, usize)>> = LazyLock::new(|| {
        let mut map = HashMap::new();
        for (i, row) in KEYPAD.iter().enumerate() {
